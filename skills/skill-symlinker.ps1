...
function Link-Global {
    ...
    try {
        $null = New-Item -ItemType SymbolicLink -Path $TargetDir -Target $SkillsSource -Force -ErrorAction Stop
        if (Test-Path $TargetDir) {
            $Item = Get-Item $TargetDir -Force -ErrorAction SilentlyContinue
            if ($Item.LinkType -eq "SymbolicLink") {
                Write-Host "[OK] $AgentName : Wired to Repo" -ForegroundColor Green
            } else {
                Write-Host "[ERROR] Created but not a symlink: $AgentName" -ForegroundColor Red
            }
        } else {
        ...
        Write-Host "Manual command: New-Item -ItemType SymbolicLink -Path `"$TargetDir`" -Target `"$SkillsSource`"" -ForegroundColor Gray
    }
}
...
# Uncomment for Gemini support:
# $GeminiDir = Join-Path $env:USERPROFILE ".gemini"
# Link-Global -TargetDir (Join-Path $GeminiDir "skills") -AgentName "Gemini"
